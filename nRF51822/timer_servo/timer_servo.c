...
static uint32_t servo_prepos = 0;
static uint32_t servo_pos = 0;
const uint32_t servo_vals = 8;
static uint32_t servo_val[] = { 5,10,15,20,25,20,15,10 };
...
//    uint32_t err_code;
    ...
    if (event_type == NRF_TIMER_EVENT_COMPARE0) {   // Interrupt from COMPARE0 event.
        ...
        } else if (tick_pos==servo_val[servo_pos]+1) {
        	nrf_gpio_pin_clear(led_pin1);
        } else if (tick_pos==201) {
		servo_prepos++;
		if (servo_prepos==100) {
			if (servo_pos<servo_vals) {
				servo_pos++;
			} else {
				servo_pos=0;
			}
...
static void timer_config(void) {
    uint32_t time_in_us = 100;   //Time(in microseconds) between consecutive compare events.
    uint32_t time2ticks;
    uint32_t err_code;
    ...
    time2ticks = nrf_drv_timer_us_to_ticks(&mytimer1, time_in_us);
