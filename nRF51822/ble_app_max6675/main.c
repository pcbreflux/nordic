...
#define DEVICE_NAME                     "nrf_max6675_gcc"                          /**< Name of device. Will be included in the advertising data. */
...
static ble_max6675_t                    m_max6675;                                  /**< MAX6675 Service instance. */

#define MAX6675_INTERVAL            APP_TIMER_TICKS(500, APP_TIMER_PRESCALER)     /**< more than MAX6675 conversion time -> interval 500ms (ticks). */
APP_TIMER_DEF(m_max6675_timer_id);                                                /**< MAX6675 timer. */
...
/**@brief Function for handling write events to the MAX6675 characteristic.
 *
 * @param[in] p_max6675  Instance of MAX6675 Service to which the write applies.
 * @param[in] led_state Written/desired state of the LED.
 */
static void blemax6675_write_handler(ble_max6675_t * p_max6675, ble_max6675_evt_t *evt) {
    uint32_t err_code;

    if (evt->evt_type == BLE_MAX6675_EVT_NOTIFICATION_ENABLED) {
    	NRF_LOG_INFO("BLE_MAX6675_EVT_NOTIFICATION_ENABLED\r\n");
        err_code = app_timer_start(m_max6675_timer_id, MAX6675_INTERVAL, NULL);
        APP_ERROR_CHECK(err_code);
        p_max6675->running=1;
    } else { // BLE_MAX6675_EVT_NOTIFICATION_DISABLED | BLE_MAX6675_EVT_DISCONNECTED
    	NRF_LOG_INFO("BLE_MAX6675_EVT_NOTIFICATION_DISABLED\r\n");
		max6675_unprepare(&max6675_config);
        err_code = app_timer_stop(m_max6675_timer_id);
        p_max6675->running=0;
        APP_ERROR_CHECK(err_code);
    }
}
...
/**@brief Function for performing max6675 measurement and updating the Battery Level characteristic
 *        in Battery Service.
 */
static void max6675_temp_update(uint16_t temp) {
    uint32_t err_code;

    err_code = ble_max6675_temp_update(&m_max6675, temp);
...
static void max6675_timeout_handler(void * p_context) {
//    uint32_t err_code;

	uint16_t temp;

	NRF_LOG_DEBUG("max6675_timeout_handler %d %u\n\r",pos);
	if (pos%2==0) {
		if (m_max6675.running!=0) {
			max6675_prepare(&max6675_config);
		}
	} else {
		if (m_max6675.running!=0) {
			temp=max6675_readcelsius(&max6675_config);
			NRF_LOG_INFO("max6675_timeout_handler temp \x1B[1;32m%u\x1B[0m,\x1B[1;31m%d\x1B[0m    \r",temp/100,temp%100);
			max6675_temp_update(temp);
		}
	}
    pos++;
}
...
static void timers_init(void) {
    ...
    // Create timers.
	err_code = app_timer_create(&m_max6675_timer_id,
								APP_TIMER_MODE_REPEATED,
								max6675_timeout_handler);
	APP_ERROR_CHECK(err_code);
}
...
static void services_init(void) {
    uint32_t       err_code;
    ble_max6675_init_t blemax6675_init;

    blemax6675_init.evt_handler = blemax6675_write_handler;

    err_code = ble_max6675_init(&m_max6675, &blemax6675_init);
    APP_ERROR_CHECK(err_code);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt) {
    ...
    ble_max6675_on_ble_evt(&m_max6675, p_ble_evt);
}
...
static void advertising_init(void) {
    ...
    ble_uuid_t adv_uuids[] = {{BLE_MAX6675_UUID_SERVICE,  m_max6675.uuid_type}}; /**< Universally unique service identifiers. */
...
int main(void) {
    ...
    NRF_LOG_INFO("start ble MAX6675\n\r");
